...
def run_tracker(tracker_name, tracker_param, run_id=None, dataset_name='otb', sequence=None, debug=0, threads=0,
    ...
    if sequence == 'train':
        train_list = [f.strip() for f in open('lib/train/data_specs/eotb_train_split.txt', 'r').readlines()]
        dataset = [dataset[i] for i in train_list]
    elif sequence == 'val':
        val_list = [f.strip() for f in open('lib/train/data_specs/eotb_val_split.txt', 'r').readlines()]
