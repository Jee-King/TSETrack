...
class EOTB(BaseVideoDataset):
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None):
        ...
        if split is not None:
            ltr_path = os.path.join(os.path.dirname(
                os.path.realpath(__file__)), '..')
            if split == 'train':
                file_path = os.path.join(
                    ltr_path, 'data_specs', 'eotb_train_split.txt')
                self.root = root + '/training_subset'
            elif split == 'val':
                file_path = os.path.join(
                    ltr_path, 'data_specs', 'eotb_val_split.txt')
                self.root = root + '/testing_subset'
            else:
    ...
    def _load_meta_info(self):
        sequence_meta_info = {s: self._read_meta(
            os.path.join(self.root, s)) for s in self.sequence_list}
    ...
    def _get_frame_path(self, seq_path, frame_id):
        ...
        # return os.path.join(seq_path, 'inter3_stack','frame{:04}_1.jpg'.format(frame_id))
        seq_name = seq_path.split('/')[-1] + '_dvs'
        return cv2.imread(os.path.join(seq_path, seq_name, 'frame{:04}.bmp'.format(frame_id)))
    
    def _get_frame(self, seq_path, frame_id):
        # return self.image_loader(self._get_frame_path(seq_path, frame_id))
        # return cv2.imread(self._get_frame_path(seq_path, frame_id))
        return self._get_frame_path(seq_path, frame_id)

    def _get_event(self, seq_path, frame_id):
        event1 = os.path.join(seq_path, 'inter3_stack',
                              '{:05}_1.jpg'.format(frame_id))
        event2 = os.path.join(seq_path, 'inter3_stack',
                              '{:05}_2.jpg'.format(frame_id))
        event3 = os.path.join(seq_path, 'inter3_stack',
                              '{:05}_3.jpg'.format(frame_id))
    ...
    def _get_frames(self, seq_id):
        path = self.coco_set.loadImgs(
            [self.coco_set.anns[self.sequence_list[seq_id]]['image_id']])[0]['file_name']
