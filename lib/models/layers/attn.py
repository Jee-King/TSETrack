...
class Inner_Attention(nn.Module):
    def __init__(self, dropout, scale, dim_hw, dim_c2):
        super(Inner_Attention, self).__init__()
        self.fc1 = nn.Linear(dim_hw, dim_c2)
        self.feat_norm1 = nn.LayerNorm(dim_c2)
        self.feat_norm2 = nn.LayerNorm(dim_hw)
        # self.fc2 = nn.Linear(self.feat_size, self.match_dim)
        self.fc3 = nn.Linear(dim_hw, dim_hw)
        self.dropout = nn.Dropout(dropout)
        self.scale = scale

    def forward(self, outermap):
        B, head_num, HW, _ = outermap.shape
        q = k = self.feat_norm1(self.fc1(outermap)) #+ pos_emb
        v = self.feat_norm2(outermap)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
        ...
        #self.inner_atten = Inner_Attention(attn_drop, self.scale, dim_hw=320, dim_c2=320)
    ...
    def forward(self, x, mask=None, return_attention=False):
        ...
        x = self.proj(x)
        # x1 = self.conv_lf(x)
        # print('dasdasdasdasdasdasd', x1.shape)
        x = self.proj_drop(x)
